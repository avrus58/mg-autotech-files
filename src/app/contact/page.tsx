...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    alternates: runtimePublicAlternates("/contact"),
    openGraph: { title: `${copy.title} | MG AutoTech`, description: copy.description, url: absoluteUrl("/contact"), siteName, type: "website", locale: runtimePublicOpenGraphLocale(locale), images: [{ url: "/opengraph-image", width: 1200, height: 630, alt: copy.title }] },
...
export default async function ContactPage() {
  ...
  const whatsappMessage = runtimePublicT(locale, "Hello MG AutoTech, I need help with a file service request.", ["core"]);
  const whatsappHref = whatsappNumber ? `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(whatsappMessage)}` : null;
  const jsonLd = localizeRuntimePublicJsonLd({ "@context": "https://schema.org", "@graph": [organizationJsonLd(locale), websiteJsonLd(locale), { "@type": "ContactPage", "@id": `${absoluteUrl("/contact")}#page`, name: title, description, url: absoluteUrl("/contact"), inLanguage: runtimePublicInLanguage(locale), mainEntity: { "@id": `${absoluteUrl("/")}#organization` } }, { "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Home", item: absoluteUrl("/") }, { "@type": "ListItem", position: 2, name: "Contact", item: absoluteUrl("/contact") }] }] }, locale, ["core"]);
  const channels = [
