...
export async function POST(request: Request) {
  ...
  if (domain === client.data.allowed_domain) return NextResponse.json({ error: "This is already the active widget domain." }, { status: 409, headers: limitHeaders });
  const conflict = await admin.from("widget_clients").select("id").eq("allowed_domain", domain).neq("id", client.data.id).neq("status", "cancelled").limit(1).maybeSingle();
