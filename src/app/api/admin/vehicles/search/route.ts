...
export async function GET(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: "Invalid vehicle search filters.", details: parsed.error.flatten() }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: "Vehicle records could not be loaded." }, { status: 500 });
  }
}
