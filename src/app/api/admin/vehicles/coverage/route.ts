...
export async function POST(request: Request) {
  ...
  if (!body || !Array.isArray(body.entries)) {
    return NextResponse.json({ error: "Invalid external coverage payload. Provide source details and entries array." }, { status: 400 });
  }
  ...
  return NextResponse.json({
    ...
    dryRun: true,
    mutation: false,
    policy: "External coverage import is source-agnostic, staging/review-first and never auto-published.",
  });
}
