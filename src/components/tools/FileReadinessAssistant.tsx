...
function readinessLabel(score: number) {
  if (score >= 82) return { title: "Ready to submit", tone: "text-emerald-300", border: "border-emerald-600/50", bg: "bg-emerald-950/20" };
  if (score >= 58) return { title: "Almost ready", tone: "text-amber-200", border: "border-amber-600/50", bg: "bg-amber-950/20" };
  return { title: "Needs preparation", tone: "text-red-200", border: "border-red-700/50", bg: "bg-red-950/20" };
}

export function FileReadinessAssistant() {
  ...
  const result = useMemo(() => {
    ...
    return { score, nextSteps, warnings, label: readinessLabel(score) };
  }, [creditStatus, fileStatus, hasFaultCodes, hasNotes, service, serviceStatus, tool, vehicleStatus]);
      ...
      <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        <div className="border border-white/10 bg-[#0b0c0e] p-5 sm:p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs font-black uppercase tracking-[0.16em] text-red-400">Readiness input</div>
              <h2 className="text-2xl font-black">Pre-check your request</h2>
            </div>
          </div>

          <div className="mt-6 grid gap-4">
            <SelectBlock label="Vehicle information" value={vehicleStatus} onChange={(value) => setVehicleStatus(value as VehicleStatus)} options={[
              ["complete", "Complete vehicle data"],
              ["partial", "Some vehicle data missing"],
              ["unknown", "Vehicle identification unclear"],
            ]} />
            <SelectBlock label="Requested service clarity" value={serviceStatus} onChange={(value) => setServiceStatus(value as ServiceStatus)} options={[
            ...
            <label className="block text-xs font-black uppercase tracking-[0.14em] text-zinc-500">
              ...
              <select value={tool} onChange={(event) => setTool(event.target.value)} className="mt-2 h-12 w-full rounded-lg border border-white/10 bg-black/40 px-4 text-sm font-bold normal-case text-white outline-none focus:border-red-700">
                {toolOptions.map((option) => <option key={option} value={option}>{option}</option>)}
              </select>
            </label>
            <SelectBlock label="Original file status" value={fileStatus} onChange={(value) => setFileStatus(value as FileStatus)} options={[
              ["ori", "Original read file is ready"],
              ["unsure", "File exists but status is unclear"],
              ["missing", "Original file not ready"],
            ]} />
            <SelectBlock label="Credits / payment" value={creditStatus} onChange={(value) => setCreditStatus(value as CreditStatus)} options={[
        ...
        <div className="space-y-4">
          <div className={`border ${result.label.border} ${result.label.bg} p-6`}>
            <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <div className="text-xs font-black uppercase tracking-[0.16em] text-zinc-400">Readiness score</div>
          ...
          {result.warnings.length > 0 && (
            <div className="border border-amber-800/40 bg-amber-950/15 p-6">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-black">Review warnings</h2>
              </div>
              <div className="mt-5 space-y-3">
