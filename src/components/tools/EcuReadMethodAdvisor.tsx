...
import type { EcuReadAdvisorCopy } from "@/lib/i18n/tool-client-copy-keys";
...
function toolT(copy: EcuReadAdvisorCopy, source: string) {
  return (copy as Readonly<Record<string, string>>)[source] ?? source;
}

function advisorTone(copy: EcuReadAdvisorCopy, score: number) {
  if (score >= 78) return { label: toolT(copy, "Good read preparation"), tone: "text-emerald-300", border: "border-emerald-600/50", bg: "bg-emerald-950/20" };
  if (score >= 52) return { label: toolT(copy, "Needs confirmation"), tone: "text-amber-200", border: "border-amber-600/50", bg: "bg-amber-950/20" };
  return { label: toolT(copy, "High review needed"), tone: "text-red-200", border: "border-red-700/50", bg: "bg-red-950/20" };
}

export function EcuReadMethodAdvisor({ copy }: { copy: EcuReadAdvisorCopy }) {
  ...
  const result = useMemo(() => {
    ...
    return { score, tone: advisorTone(copy, score), primaryMethod, checklist, warnings };
  }, [copy, fileStatus, hasBatterySupport, hasEcuLabel, hasFaultCodes, readAccess, toolStatus, vehicleType]);
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-14 sm:py-20">
      ...
      <div className="mt-6 flex items-start gap-3 border-l-2 border-emerald-500 bg-emerald-950/10 p-4 text-sm leading-6 text-emerald-100">
        ...
        {t("Customer safety: no file picker, no upload session, no binary analysis, no checksum, no file generation.")}
      </div>
    </section>
  );
}
