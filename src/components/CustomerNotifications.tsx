...
function timeLabel(value: string, locale: LocaleCode) {
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return "";
  const relative = new Intl.RelativeTimeFormat(intlLocaleByCode[locale], {
    numeric: "auto",
    style: "short",
  });
  ...
  return date.toLocaleDateString(intlLocaleByCode[locale], {
    day: "2-digit",
    month: "2-digit",
  });
}
              ...
              {notificationLoading && items.length === 0 ? (
                ...
                <Link key={item.id} href={item.order_id ? `/dashboard/orders/${item.order_id}` : "/dashboard"} onClick={() => markRead([item.id])} className={`block border-b border-white/5 px-4 py-4 transition hover:bg-white/[0.05] ${item.read_at ? "opacity-60" : "bg-red-950/10"}`}>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1"><div className="break-words font-black text-white" translate={copy.rawTitle ? "no" : undefined} data-no-translate={copy.rawTitle ? true : undefined}>{copy.title}</div>{copy.body && <div className="mt-1 line-clamp-2 break-words text-sm leading-5 text-zinc-400" translate={copy.rawBody ? "no" : undefined} data-no-translate={copy.rawBody ? true : undefined}>{copy.body}</div>}<div className="mt-2 text-xs text-zinc-600">{timeLabel(item.created_at, locale)}</div></div>
                  </div>
                </Link>
                );
              })}
