...
export const fileExpertReportRows = [
  ...
  ["versionLegacy", "Legacy version", "Ältere Version", "Eski sürüm", "Oudere versie", "Version antérieure", "Versione precedente", "Versión anterior", "Versão anterior", "Starsza wersja", "Предыдущая версия", "旧版本", "Version i mëparshëm"],
  ...
  ["vehicleMultiple", "{target} appears in {count} vehicle applications. ECU identity alone does not prove the exact vehicle or engine.", "{target} kommt in {count} Fahrzeuganwendungen vor. Die ECU-Identifikation allein belegt weder das genaue Fahrzeug noch den Motor.", "{target}, {count} araç uygulamasında bulunuyor. Yalnızca ECU tanımlaması kesin aracı veya motoru kanıtlamaz.", "{target} komt voor in {count} voertuigtoepassingen. Alleen de ECU-identificatie bewijst niet het exacte voertuig of de motor.", "{target} apparaît dans {count} applications véhicule. L’identification de l’ECU seule ne prouve ni le véhicule ni le moteur exact.", "{target} compare in {count} applicazioni veicolo. La sola identificazione ECU non dimostra il veicolo o il motore esatto.", "{target} aparece en {count} aplicaciones de vehículo. La identificación de la ECU por sí sola no demuestra el vehículo ni el motor exactos.", "{target} aparece em {count} aplicações de veículo. A identificação da ECU, por si só, não prova o veículo nem o motor exatos.", "{target} występuje w {count} zastosowaniach pojazdu. Sama identyfikacja ECU nie potwierdza dokładnego pojazdu ani silnika.", "{target} встречается в {count} вариантах автомобиля. Одна лишь идентификация ECU не подтверждает точную модель или двигатель.", "{target} 出现在 {count} 个车型应用中；仅凭 ECU 识别信息无法证明具体车辆或发动机。", "{target} shfaqet në {count} zbatime automjeti. Vetëm identifikimi i ECU-së nuk vërteton automjetin ose motorin e saktë."],
