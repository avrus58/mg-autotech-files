...
create table if not exists public.file_expert_jobs (
  ...
  ecu_type text nullable,
  read_method text nullable,
  customer_notes text nullable,
  ori_file_path text nullable,
  mod_file_path text nullable,
  ori_file_name text nullable,
  mod_file_name text nullable,
  ori_sha256 text nullable,
  mod_sha256 text nullable,
  ori_file_size bigint nullable,
  mod_file_size bigint nullable,
  result_json jsonb nullable,
  ai_report text nullable,
  executive_summary text nullable,
  detected_features jsonb nullable,
...
create table if not exists public.file_expert_feedback (
  ...
  admin_user_id uuid nullable references auth.users(id) on delete set null,
  actual_features jsonb nullable,
  ai_correct boolean nullable,
  quality_rating int nullable check (quality_rating is null or (quality_rating between 1 and 5)),
  safety_rating text nullable check (safety_rating is null or safety_rating in ('safe', 'aggressive', 'risky', 'unknown')),
...
create table if not exists public.known_file_patterns (
  id uuid primary key default gen_random_uuid(),
  ecu_family text nullable,
  ecu_type text nullable,
  sw_number text nullable,
  hw_number text nullable,
  feature_type text nullable,
  pattern_signature jsonb not null,
  source_job_id uuid nullable references public.file_expert_jobs(id) on delete set null,
...
create table if not exists public.file_expert_binary_fingerprints (
  ...
  ecu_strings jsonb nullable,
  ascii_strings jsonb nullable,
  ff_ratio numeric nullable,
  zero_ratio numeric nullable,
  entropy numeric nullable,
  active_regions jsonb nullable,
  fingerprint_json jsonb nullable,
  created_at timestamptz default now()
);
