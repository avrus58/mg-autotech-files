-- Idempotent and cross-account-safe growth attribution delivery.
-- Additive: no customer or attribution rows are removed.

begin;

create extension if not exists pgcrypto;

-- These two prerequisite tables historically lived in an operator script.
-- Keep the migration replayable on a clean staging database as well as on
-- installations where the original additive script was already applied.
...
-- `create table if not exists` does not update the prior operator-script
-- constraint. Replace only this allowlist constraint so existing installations
-- can accept the consented, auth-bound identity link event as well.
...
-- The former application preferred a configured dedicated secret and otherwise
-- fell back to the service-role key. SQL cannot determine which key produced an
-- existing one-way hash, so label pre-cutover rows explicitly as unknown. The
-- application dual-reads both candidates only for those existing rows.
...
revoke all on function public.touch_growth_customer_success_updated_at()
...
revoke all on function public.record_growth_attribution_touch(text, text, text, uuid, text, text, text, text, text, text, text, text, text)
  from public, anon, authenticated;
grant execute on function public.record_growth_attribution_touch(text, text, text, uuid, text, text, text, text, text, text, text, text, text)
  to service_role;
...
do $$ begin
  if to_regprocedure('public.record_growth_attribution_touch(text,uuid,text,text,text,text,text,text,text,text,text)') is not null then
    -- Keep the previous app version rollback-capable for this release. The
    -- legacy overload remains server-only and can be revoked by a later,
    -- separately verified cutover after the application rollback window closes.
